/**
 * 【文件职责】实现 `@earendil-works/pi-protocol` 包中的 `cbor/index` 模块，集中维护该模块的类型、状态与操作入口。
 * 【技术维度】主要依赖 `./decoder.ts`、`./encoder.ts`、`./options.ts`，并通过 TypeScript 模块边界组织实现。
 * 【产品维度】为远程 pi 会话定义传输无关的 CBOR 通信协议；本文件负责其中与 `cbor/index` 对应的子能力。
