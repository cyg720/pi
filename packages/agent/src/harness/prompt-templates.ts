/**
 * 【文件职责】实现 `@earendil-works/pi-agent-core` 包中的 `harness/prompt-templates` 模块，集中维护该模块的类型、状态与操作入口。
 * 【技术维度】主要依赖 `yaml`、`./types.ts`，并通过 TypeScript 模块边界组织实现。
 * 【产品维度】为通用智能体提供传输抽象、状态管理与附件能力；本文件负责其中与 `harness/prompt-templates` 对应的子能力。
 * 【逻辑维度】对外入口包括 `PromptTemplateDiagnosticCode`、`PromptTemplateDiagnostic`、`loadPromptTemplates`、`loadSourcedPromptTemplates`、`parseCommandArgs`、`substituteArgs`；内部辅助逻辑围绕这些入口完成数据转换与流程控制。
