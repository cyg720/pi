 ...
 */
/**
 * 文件职责：提供 HTML 导出功能使用的 Highlight.js 浏览器端语法高亮运行时与内置语言规则。
 * 技术维度：这是 Highlight.js 11.9.0 的压缩 UMD 构建，包含解析器、HTML 发射器和多语言正则语法定义。
 * 产品维度：让导出的对话 HTML 能在离线环境中高亮代码块，无需额外网络脚本。
 * 逻辑维度：前半部分实现模式编译、词法扫描和 HTML 输出，后半部分注册 Bash、JavaScript、TypeScript 等语言。
 * 关键边界：该文件是带 BSD-3-Clause 许可证的供应商产物；压缩符号会在同一表达式内复用，不能按普通源码重命名或重排。
 * 新手阅读建议：不要从单字母变量逐行入门；先查 Highlight.js 11.9.0 上游未压缩源码，再对照末尾语言注册和 module.exports。
 */
/**
 * 顶层变量 hljs 保存完整的高亮运行时；立即执行函数返回公共 API，在 CommonJS 环境下还会赋给 module.exports。
 * 内部单字母函数、类、局部变量和常量均由上游压缩器生成，其用途与取值范围以对应未压缩模块为准；示例：hljs.highlight(code, { language })。
 */
