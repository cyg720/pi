...
/**
 * 文件职责：提供 vendored Marked Markdown 解析器，把会话 Markdown 转换为导出 HTML 所需的令牌与标记。
 * 技术维度：使用 Marked 18 的 UMD 构建、正则词法分析、块级/行内 Tokenizer、Renderer、Parser 与扩展钩子。
 * 产品维度：让 HTML 导出无需运行时安装外部包即可稳定渲染标题、列表、代码块、表格、链接等内容。
 * 逻辑维度：定义规则与转义工具，完成词法分析和令牌化，再由渲染器与解析器生成 HTML，并导出公共 API。
 * 关键边界：这是第三方生成压缩文件，受 MIT 许可证约束；任何手工内部改动都会在升级或重新生成时丢失。
 * 新手阅读建议：优先阅读上游 Marked 源码；本文件只需识别 Tokenizer、Lexer、Parser、Renderer 和最终导出映射。
 */
 ...
 * 中文说明：本文件由上游 ./src/ 源码生成，压缩类、函数、变量与常量均应在上游源码理解和修改。
 */
// 生成代码元素说明：以下短名称是压缩器生成的内部符号；它们分别服务于规则、分词、解析、渲染和扩展机制，不应作为稳定接口使用。
